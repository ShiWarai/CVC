...
async def load_from_hf(request: LoadFromHfRequest):
    ...
    if not repo_id:
        ...
        if not repo_id:
            raise HTTPException(
                status_code=400,
                detail="repo_id не указан в запросе и HF_REPO_ID не найден в конфигурации сервера",
            )
    ...
    if request.local_dir:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="local_dir должен быть относительным путем внутри рабочей директории",
            )
    ...
    thread = threading.Thread(
        target=_run_load_from_hf_task, args=(repo_id, local_dir, load_id), daemon=True
    )
    thread.start()
