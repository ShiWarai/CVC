...
def load_config(config_path: str = "config.yaml") -> dict:
    ...
    if Path("/app").exists():
        config_file = (
            Path("/app") / "config.yaml" if config_path == "config.yaml" else Path(config_path)
        )
    else:
...
def main():
    ...
    num_iterations = (
        int(num_iterations_env) if num_iterations_env else training_config.get("iterations", 20)
    )
    ...
    try:
        ...
        from app.db import mark_examples_as_trained
